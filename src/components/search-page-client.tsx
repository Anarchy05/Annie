    ...
    <PageShell>
      <section className="rounded-3xl border border-[#2A2A3E] bg-[#1A1A2E]/80 p-5 shadow-[0_20px_60px_rgba(0,0,0,0.35)]">
        ...
        {recentSearches.length ? (
          <div className="mt-4 rounded-2xl border border-[#2A2A3E] bg-black/20 p-4">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <div>
                <p className="text-xs uppercase tracking-[0.18em] text-white/35">Recent jumps</p>
                <p className="mt-1 text-sm text-white/60">Pick up a thread Annie helped with recently.</p>
              </div>
              <button
                type="button"
                onClick={() => clearRecentSearches(setRecentSearches)}
                className="rounded-full border border-white/10 bg-black/20 px-3 py-1.5 text-xs text-white/65 transition hover:border-white/20 hover:text-white"
              >
                Clear recents
              </button>
            </div>
...
function clearRecentSearches(setRecentSearches: Dispatch<SetStateAction<string[]>>) {
  setRecentSearches([]);
